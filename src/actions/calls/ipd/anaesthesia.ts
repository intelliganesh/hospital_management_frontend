...
import {
  ANAESTHESIA_LIST_URL,
  ANAESTHESIA_DETAIL_URL,
...
export const useAnaesthesia = () => {
  const dispatch = useDispatch();

  const addAnaesthesiaHandler = async <T>(
    data: T,
  ...
  const anaesthesiaDetailHandler = async (
    id: string,
    callback: ApiCallback,
    data?: any,
    isLoading?: (status: LoadingStatus) => void,
    ...
    try {
      await api.get(
        ANAESTHESIA_DETAIL_URL + "/" + id,
        (response: AuthPayload, success: boolean, statusCode: number) => {
          if (success && statusCode === 200) {
            dispatch(anaesthesiaDetailSlice(response.data));
            return callback(true);
          } else {
            response && handleApiError(response);
            return callback(false);
  ...
  const editAnaesthesiaHandler = async (
    id: string,
    data: any,
    callback: ApiCallback,
  ): Promise<void> => {
