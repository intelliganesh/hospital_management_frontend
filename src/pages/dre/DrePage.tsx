...
const DrePage: React.FC<{}> = () => {
  ...
  const { dreListHandler, deleteDreHandler, cleanUp } = useDre();
  const [searchParams, setSearchParams] = useSearchParams();
  const [deleteId, setDeleteId] = useState<null | string>(null);
    const [isDeleting, setIsDeleting] = useState(false);
  
  const dreListData = useSelector((state: any) => state?.dre?.dreListData);
  ...
  const sortOptions: SortOption[] = [
    { label: "Dre Name (A-Z)", value: "dre_name", order: "asc" },
    { label: "Dre Name (Z-A)", value: "dre_name", order: "desc" },
  ...
  return (
    <React.Fragment>
      ...
      >
        <View className="flex justify-end gap-2">
          ...
          <Button
            ...
            onPress={() => {
              if (deleteId) {
                deleteDreHandler(deleteId, (success: boolean) => {
                  if (success) {
                    dreListHandler(
                      searchParams?.get("currentPage") ?? 1,
                      () => {
      ...
      <Card className={dynamicTableCardStyle}>
        ...
        {/* Table */}
        <DynamicTable
          tableHeaders={["Dre Name", "Department Type", "Actions"]}
          tableData={dreListData?.data?.map((data: any) => [
            // <Link
            ...
            <ActionMenu
              onEdit={() =>
                navigate(DRE_TABLE_URL + DRE_EDIT_URL + "/" + data.id)
              }
          ...
          header={{
            ...
            action: (
              <Button
                ...
                size="small"
                onPress={() => {
                  navigate(DRE_TABLE_URL + DRE_FORM_URL);
                }}
          ...
          footer={{
            pagination: (
              <PaginationComponent
                current_page={dreListData?.current_page}
                last_page={dreListData?.last_page}
