...
const ProctoscopyForm: React.FC<FormTypeProps> = ({
  ...
  const proctoscopyData = useSelector(
    (state: any) => state.proctoscopy.proctoscopyDetailData
  );
  const departmentType = iAmIn ? useSelector(
    (state: any) => state?.consultation?.consultationDetailData
  ).consultations?.type : "";
  const { values, handleChange, onSetHandler } = useForm<Proctoscopy | null>(
    proctoscopyData
  );
      ...
      <View className="bg-white dark:bg-slate-800 rounded-xl shadow-soft dark:shadow-none border border-slate-200 dark:border-slate-700 w-full max-w-4xl p-6 md:p-8 mb-8">
        <View className="flex items-center justify-between mb-6">
          <View>
            ...
            >
              {formType === "add" ? "New Proctoscopy" : "Edit Proctoscopy"}
            </Text>
