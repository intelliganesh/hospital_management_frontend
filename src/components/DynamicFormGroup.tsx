...
const DynamicFormGroup: React.FC<DynamicFormGroupProps> = ({
  ...
  useEffect(() => {
    // If data passed in has items, ensure they have IDs
    // If empty and minGroups > 0, initialize defaults
    if (data && data.length > 0) {
      ...
      // simple check: length or deep compare. For now, rely on parent stability or just set
       // To avoid infinite loops, we can check if JSON stringify matches exactly (ignoring new IDs creation if unnecessary)
       // But typically `data` prop reference changes. 
       // Let's blindly trust the prop for now but keeping our own IDs persistent if possible would be better.
       // For this implementation, we re-map.
       setInternalData(ensuredData);
    } else if (minGroups > 0 && (!data || data.length === 0)) {
       // Initialize valid empty groups
       const initialGroups = Array.from({ length: minGroups }).map((_, idx) => createEmptyItem(`init-${idx}`));
       setInternalData(initialGroups);
       // We should verify if we need to call onChange here to sync up parent.
       // Ideally yes, but inside useEffect is risky. Let's wait for user interaction or parent initialization.
    } else {
        setInternalData([]);
    }
  }, [data, minGroups]);
  ...
  const handleRemoveGroup = useCallback((id: string) => {
    ...
    // If isDeletable is defined, we should ideally check it here too, but passing item/index is hard without lookup.
    // relying on UI disabled state is usually enough for the 'button' trigger.
  ...
  const handleFieldChange = useCallback((id: string, fieldKey: string, value: any) => {
  ...
  const renderFieldInput = (item: DynamicItem, field: FieldConfig) => {
      if (renderCustomField) {
          const custom = renderCustomField(field, item, (val) => handleFieldChange(item.id, field.key, val));
          if (custom) return custom;
      }

    //   const error = validationErrors[item.id]?.[field.key];
      const fieldError = validationErrors[item.id]?.[field.key]; // Local validation
      // You might also want to look at `errors` prop if it maps to specific rows, 
      // but usually dynamic lists need their own ID-based error tracking or an index-based logic from parent.
          ...
          case 'textarea':
              return (
                  <Textarea
                      ...
                      onChange={(e: any) => handleFieldChange(item.id, field.key, e.target.value)}
                  />
              );
          case 'select':
              return (
              ...
              return (
                  <View>
                      ...
                      <RadioGroup
                          ...
                          {...field.componentProps}
                      />
                       {fieldError && <Text className="text-red-500 text-sm mt-1">{fieldError}</Text>}
                  </View>
              );
          case 'checkbox':
              return (
                 <View className="flex items-center gap-2 pt-8"> 
                     ...
                     <CheckBox
                         ...
                         onChange={(e: any) => handleFieldChange(item.id, field.key, e.target.checked)}
                     />
                      <label htmlFor={commonProps.id} className="text-sm font-medium text-slate-700 dark:text-slate-300 cursor-pointer">
                          {field.label}
                      </label>
                 </View>
              );
  ...
  return (
    <Card className={`w-full bg-white dark:bg-slate-900 shadow-sm border border-slate-200 dark:border-slate-800 ${className}`}>
        <CardHeader className="pb-4 border-b border-slate-100 dark:border-slate-800">
            <View className="flex flex-row items-center justify-between">
                ...
                <Text className="text-xs text-slate-500 font-normal">
                    {internalData.length} {internalData.length === 1 ? entryLabel : entryLabel + 's'}
                </Text>
            </View>
        </CardHeader>
        <CardContent className="space-y-6 pt-6">
                ...
                >
                    ...
                    >
                        {fields.map((field) => (
                            <div 
                                key={field.key} 
                                style={{ gridColumn: field.colSpan ? `span ${field.colSpan} / span ${field.colSpan}` : 'auto' }}
                            >
            ...
            {!readOnly && internalData.length < maxGroups && (
                <Button variant="outline" onPress={handleAddGroup} className="w-full flex items-center justify-center border-dashed border-2 py-6 hover:border-blue-500 hover:text-blue-600 dark:hover:border-blue-400 dark:hover:text-blue-400 transition-all group">
